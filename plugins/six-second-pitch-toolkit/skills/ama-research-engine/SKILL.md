...
**State the offer-viability verdict before writing the pitch, not just the research (added v1.17.0, 2026-09-02, folded in from the external SSP Coach v1.0.1 review, Dee's call).** Finishing AMA4+AMA5 is not the same claim as "the message is the problem worth fixing." After the research runs and before SSP SOP writes anything, say plainly which of these is true: **Message problem** (Person, Problem, or Promise is unclear), **Trust or path problem** (people understand it but proof or the next step is weak), **Offer problem** (the price, format, product, or delivery doesn't fit the evidence), or **Evidence problem** (not enough reliable research yet to decide). Do not let the pitch get rewritten when the evidence points somewhere else — that edits the fruit while the root stays broken.
...
**A file marked "secondary-source VOC" or "does not satisfy the AMA4 gate" does NOT satisfy the AMA4 gate — pull fresh VOC before writing the headline, every time (added v1.16.0, 2026-08-25, Dee's call).** A VOC-shaped file on disk is not the same claim as "AMA4 ran." On 2026-08-25, a Petty Powers printable was drafted for a headline using `_Research/2026-08-18-EXTERNAL-perplexity-VOC-teen-girls-adhd-autism-audhd-anxiety.md` — a file whose own header says in plain words: *"This is secondary-source VOC, not an AMA4 run. It does not satisfy the AMA4+AMA5 copy gate."* That sentence was read and the draft proceeded anyway. The result, "STOP REPLAYING IT.", failed the native-tongue test; a fresh pull one round later ("am I overthinking this," "left on read," teens' own phrasing) is what actually worked. **How to run it:** before writing ANY headline, title, or hook, check whether the VOC file backing it is marked primary (a real AMA4 run: fresh reviews/forum/Reddit pull, this session or recent) or secondary (someone else's research, a summary, an external AI's prior pass). If it's marked secondary, or you cannot find a primary-VOC citation for the specific headline you're about to write, **stop and pull fresh audience language first** — a live search for how the actual audience phrases the actual moment, not framework language you invented. This applies to the headline specifically even when body copy elsewhere in the same piece is fine on secondary VOC; the headline is where a stranger's cold read happens, and that is exactly where this failed twice in one session.
...
- **Rate the research's Evidence Confidence before handing it to SSP SOP (added v1.17.0, 2026-09-02, folded in from the external SSP Coach v1.0.1 review, Dee's call).** Label the AMA4/AMA5 findings **Confirmed, Promising, Weak, or Unknown**, and say what would raise the rating. A pile of cited quotes is not the same claim as validated demand; the label is what keeps that distinction alive into the pitch-writing step instead of getting flattened into "the research is done."
